# Unisence IoT：轻松支持百万设备、单机 30 万 QPS 的高性能物联网系统

[测试报告](docs/performance-benchmark.md) · 单机全链路 300,004 QPS / 5 分钟稳定通过

> 面向工业设备的高性能物联网平台，覆盖设备建模、数据接入、时序存储、在线状态和规则计算。

**单机全链路实测轻松稳定处理 30 万 QPS**（[测试报告](docs/performance-benchmark.md)）。Unisence IoT 将管理控制台、设备接入、Kafka
...
当前单机、单 Kafka Broker、单 engine、单 rule-stream 的全链路压测中（环境、负载模型、LAG
曲线与对账见[测试报告](docs/performance-benchmark.md)）：
